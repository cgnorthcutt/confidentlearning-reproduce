...
def noisify_from_file(y_train, filename, train_mask_dir=None):
    ...
    train_mask_dir : a npy file containing a bool np.array'''
...
#             transforms.Normalize((0.4914, 0.4822, 0.4465), (0.2023, 0.1994, 0.2010)),
    ...
    if train_mask_dir:
        orig_class_counts = np.bincount(true_labels)
        train_bool_mask = np.load(train_mask_dir)
        # Mask labels
        train_dataset.imgs = [img for i, img in enumerate(train_dataset.imgs) if train_bool_mask[i]]
        clean_class_counts = np.bincount([l for _, l in train_dataset.imgs])
        ...
        class_weights = torch.Tensor(orig_class_counts / clean_class_counts).to(device)
    ...
    train_loader = torch.utils.data.DataLoader(
        train_dataset, batch_size=60000, shuffle=False,
        num_workers=10, pin_memory=True, sampler=None,
    )
    # Extract data and labels from loader
    ...
    diff = np.sum([v != true_labels[i] for i, v in enumerate(train_noisy_labels)])
    actual_noise = diff / float(len(true_labels))
    assert actual_noise > 0.0
    print('Actual noise %.2f' % actual_noise)
    
    return np.asarray([[i] for i in train_noisy_labels]), train_data, actual_noise, class_weights
    

def noisify(dataset='mnist', nb_classes=10, train_labels=None, noise_type=None, noise_rate=0, random_state=0, noise_filename=None, train_mask_dir=None):
    ...
    if noise_type == 'from_file':
        return noisify_from_file(train_labels, noise_filename, train_mask_dir)
