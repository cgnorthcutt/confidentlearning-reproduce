...
def noisify_from_file(y_train, filename):
...
#     print(type(train_data), type(train_noisy_labels), train_data.shape, train_noisy_labels.shape)
    ...
    return np.asarray([[i] for i in train_noisy_labels]), np.asarray([[i] for fn, i in train_dataset.imgs]), train_data, actual_noise
...
def noisify(
    ...
    if noise_type == 'from_file':
        return noisify_from_file(train_labels, noise_filename)
